...
with tf.Graph().as_default():
    with tf.variable_scope('mnet'):
        ...
        with tf.name_scope('Inference'):
            ...
            with tf.name_scope('FC_Linear'):
                ...
                biases2=BiasesVariable(shape=[n_classes],name_str='biases',is_train=False)
    ...
    with tf.name_scope('AddTestLayer'):
        ...
        with tf.name_scope('Evaluate'):
            correct_pred=tf.equal(tf.argmax(ty,1),tf.argmax(Y_true,1))
    ...
    mnet_var = tf.get_collection(tf.GraphKeys.VARIABLES, scope='mnet')
    print(len(mnet_var))
    ...
    with tf.Session() as sess:
        #载入模型参数值
        sess.run(init)
        saver.restore(sess, tf.train.latest_checkpoint('ckptfilesScoped/'))
